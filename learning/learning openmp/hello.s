...
# options passed:  -fpreprocessed hello.i -mtune=generic -march=x86-64
...
# hello.c:8:     int sum = 0;
	movl	$0, -8(%rbp)	#, sum
# hello.c:9:     for (int i = 0; i < ARRAYSIZE; i++)
	movl	$0, -4(%rbp)	#, i
# hello.c:9:     for (int i = 0; i < ARRAYSIZE; i++)
	jmp	.L2	#
.L3:
...
# hello.c:9:     for (int i = 0; i < ARRAYSIZE; i++)
	...
	jle	.L3	#,
# hello.c:13:     t1 = omp_get_wtime();
	call	omp_get_wtime@PLT	#
	movq	%xmm0, %rax	#, tmp89
	movq	%rax, -16(%rbp)	# tmp89, t1
# hello.c:14: #pragma omp parallel for default(shared) reduction(+ \
...
# hello.c:19:     t2 = omp_get_wtime();
	call	omp_get_wtime@PLT	#
	movq	%xmm0, %rax	#, tmp93
	movq	%rax, -24(%rbp)	# tmp93, t2
# hello.c:21:     printf("Time taken = %g\n", t2 - t1);
