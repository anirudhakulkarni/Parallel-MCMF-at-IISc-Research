...
namespace boost
{
    struct SampleGraph
    {
        ...
        static tuple<Graph,vertex_descriptor,vertex_descriptor> getSampleGraph(
            Graph g, vertex_descriptor s, vertex_descriptor t, vector<vector<int>> sources,vector<vector<int>> sinks, vector<vector<int>> edges)
        {
            ...
            return getSampleGraph(g, s, t, capacity, residual_capacity, weight, rev,sources,sinks,edges);
        }

        template <class Graph, class Weight, class Capacity, class Reversed,
                  class ResidualCapacity>
        static tuple<Graph,vertex_descriptor,vertex_descriptor> getSampleGraph(Graph g, vertex_descriptor s,
                                   ...
                                   ResidualCapacity residual_capacity, Weight weight, Reversed rev,vector<vector<int>> sources,vector<vector<int>> sinks, vector<vector<int>> edges)
        {
            ...
            {
                ea.addEdge(edges[i][0],edges[i][1],edges[i][2],999999);
            }
            for (int i = 0; i < sinks.size(); i++)
            {
                ea.addEdge(sinks[i][0],t,0,sinks[i][1]);
            }
            return make_tuple(g,s,t);
            // ea.addEdge(0, 1, 4, 2);
            // ea.addEdge(0, 2, 2, 2);
